--[[
		...
		Remade and optimized by Riky47#3355 for vihieu X
		# spdmteam.com
...
local Script_Title = "vihieu X <font color=\"rgb(255, 0, 0)\">|</font> Mobile fly"


-- Instances:
local vihieu = Instance.new("ScreenGui")
...
vihieu.Name = "vihieu"
vihieu.Enabled = true
vihieu.ResetOnSpawn = true
vihieu.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
vihieu.DisplayOrder = 999999999
...
Logo.Size = UDim2.new(40, 0, 40, 0)
Logo.ZIndex = 2
Logo.Image = "http://www.roblox.com/asset/?id=131983967040060"
...
Toggle.Name = "fly gui vihieu nhấn để bay"
Toggle.Visible = false
--Toggle.Parent = vihieu
...
local success, err = pcall(function()
	vihieu.Parent = uiparent()
end)

if not success then
	vihieu.Parent = game:GetService("Players").LocalPlayer.PlayerGui
end
...
local incraseBtn
incraseBtn = addButton("cộng 1 tốc độ", function()
	local char = speaker.Character
...
local decraseBtn
decraseBtn = addButton("trừ 1 tốc độ", function()
	local char = speaker.Character
	if char and char.Humanoid then
		local hum = char.Humanoid
...
local goUp
goUp = addButton("a long", function()
	local char = speaker.Character
	if char then
